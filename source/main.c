...
int main (int argc, char * const argv[], char * const argp[])
{
		...
		switch (iargs)
		{
			...
			case 'l':
				if(isNumber(optarg) == 1)
				{
					...
					/**
					 * Если указана нулевая длина, то будет запрошен ввод с клавиатуры.
					 * Поэтому игнорируем ключ -c[--count].
					 */
					if (atoi(optarg) > 0) {
						useCount = 1;
					}
			...
			case 'c':
				if(isNumber(optarg) == 1)
				{
					...
					if (atoi(optarg) > MAX_PASSWORD_COUNT) {
						Print.error("%s: %d", _("Max password count"), MAX_PASSWORD_COUNT);
					}
			...
			case 'P':
				...
				if (strlen(optarg) > MAX_PATTERN_LENGTH) {
					Print.error("%s: %d", _("Max pattern length"), MAX_PATTERN_LENGTH);
				}
			...
			case 'e':
				...
				if ((strlen(source) + 1) > MAX_PATTERN_LENGTH) {
					...
					sprintf(__e__, _("No use key %s with long pattern"), ArgKeys[9].manstr);
					Print.error("%s: %d (%s)", _("Max pattern length"), MAX_PATTERN_LENGTH, __e__);
				}
				strcat(source, " ");
				break;
			case '?':
	...
	*/
	...
	if (strlen(source) > 0)
	{
		...
		sprintf(__s__, "%s, %s, %s, %s, %s", ArgKeys[4].manstr, ArgKeys[5].manstr, ArgKeys[6].manstr, ArgKeys[7].manstr, ArgKeys[8].manstr);
